...
public class ServiceDiscoveryBuilder<T>
{
     ...
     *
     * @param payloadClass the class of the payload of your getGroup instance (you can use {@link Void}
    ...
    /**
     * Build a new getGroup discovery with the currently set values. If not set, the builder will be
     * defaulted with a {@link JsonInstanceSerializer}.
     *
     * @return new getGroup discovery
     */
    public ServiceDiscovery<T> build()
    {
        ...
        if ( serviceDefinitionSerializer == null)
        {
            serializer(new FastjsonServiceDefinitionSerializer<>(payloadClass));
        }
        return new ServiceDiscoveryImpl<T>(client, basePath, serializer, serviceDefinitionSerializer, thisInstance, watchInstances);
    }
     ...
     * 服务描述序列化器
     */
    private ServiceDefinitionSerializer<T> serviceDefinitionSerializer;

    /**
     * optional - only needed when you use {@link ServiceCache#getNewestServiceDefinition()}
     * added by happyyangyuan
     */
    public ServiceDiscoveryBuilder<T> serializer(ServiceDefinitionSerializer<T> payloadSerializer)
    {
        this.serviceDefinitionSerializer = payloadSerializer;
        return this;
    }

    /**
     * Optional - instance that represents the getGroup that is running. The instance will get auto-registered
     *
